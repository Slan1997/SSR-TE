...
p_load(mvtnorm, # generate multivariate normal
       #ldbounds, # Lan-DeMets bounds with efficacy only
       gsDesign, # Lan-DeMets bounds with both efficacy and futility
...
for (s in (1:sub_nsim)){
  ...
  }else{ # Cfut1 <= bar_Z_star1 <= C1, do not reject H0
...
write_csv(output,paste0(getwd(),"/result_power_perm729/",'perm',run_idx,'.csv'))
